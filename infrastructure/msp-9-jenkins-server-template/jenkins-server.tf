...
resource "aws_instance" "tf-jenkins-server" {
  ...
  user_data = file("jenkinsdata.sh")  # jenkins i ayaga kaldiracak configuration burada
}
...
resource "aws_iam_role" "tf-jenkins-server-role" {
  # jankins e role atayarak yetki vermemiz gerekiyor. Burada bu islemi yapiyoruz
